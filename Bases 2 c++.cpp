...
void editarArchivo(const string& nombreArchivo) {
    string guardado;
    cout << "Ingrese el nombre del archivo que desea editar: ";
    cin.ignore();
    getline(cin, guardado);
    ...
    ifstream archivoLectura(guardado);
    ...
    cout << "\n📜 Contenido actual del archivo '" << guardado << "':\n";
    for (size_t i = 0; i < lineas.size(); i++) {
        cout << i + 1 << ": " << lineas[i] << endl;
    }

    // Pedir qué línea editar
    int numLinea;
    cout << "\nIngrese el número de la línea que desea editar (0 para cancelar): ";
    cin >> numLinea;
    cin.ignore();

    if (numLinea <= 0 || numLinea > (int)lineas.size()) {
        cout << "❌ Número de línea inválido o cancelado." << endl;
        return;
    }

    // Pedir el nuevo texto
    cout << "Ingrese el nuevo contenido para la línea " << numLinea << ": ";
    string nuevoTexto;
    ...
    ofstream archivoEscritura(guardado, ios::out | ios::trunc);
    for (size_t i = 0; i < lineas.size(); ++i) {
        archivoEscritura << lineas[i] << '\n';
    }
